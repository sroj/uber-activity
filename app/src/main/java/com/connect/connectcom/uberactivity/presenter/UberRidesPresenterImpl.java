...
import com.connect.connectcom.uberactivity.ConnectivityStatusManager;
import com.connect.connectcom.uberactivity.FusedLocationManager;
import com.connect.connectcom.uberactivity.Ln;
 ...
 */
public class UberRidesPresenterImpl implements UberRidesPresenter {

    private FusedLocationManager fusedLocationManager;

    private UberAPI uberAPI;

    private UberRidesView uberRidesView;
    ...
    public void onRideSelected(UberRide uberRide) {
        uberRidesView.launchUberActivity(
                uberRide
        );
    }
    ...
    public void loadUberRides(final double dropOffLatitude, final double dropOffLongitude) {
        ...
        subscribe = fusedLocationManager.getLastKnownLocationObservable().observeOn(AndroidSchedulers.mainThread()).flatMap(
                new Func1<Location, Observable<List<UberRide>>>() {
                    ...
                    public Observable<List<UberRide>> call(final Location location) {
                        if (location != null) {
                            ...
                            uberRidesView.drawPickupMarker(location.getLatitude(), location.getLongitude());
                            uberRidesView.drawDropOffMarker(dropOffLatitude, dropOffLongitude);

                            Observable<Products> fetchProducts = uberAPI.getProducts(
                            ...
                            Observable<Prices> fetchPrices = uberAPI.getPrices(
                                    location.getLatitude(),
                                    location.getLongitude(),
                                    dropOffLatitude,
                                    dropOffLongitude
                            ...
                            Observable<Times> fetchTimes = uberAPI.getTimes(
                                    location.getLatitude(),
                                    location.getLongitude()
                            ).doOnError(
                                    new Action1<Throwable>() {
                                        ...
                                        public List<UberRide> call(
                                            ...
                                            if (products.isOk() && times.isOk() && prices.isOk()) {
                                                ...
                                                if (!((numProducts == numPrices) && numPrices == numTimes)) {
                                                    Ln.w(
                                                            "HEY! The Uber API responses had different number of elements"
                                                    );
                                                }
                ...
                new Subscriber<List<UberRide>>() {
                    ...
                    public void onError(Throwable e) {
                        Ln.e(
                                "Error while obtaining the initial network and location data: %s",
                                e
                        );
                        if (e.getCause().getCause() instanceof ConnectivityStatusManager.NoConnectivityException) {
                            uberRidesView.showNoConnectivity();
                        }
                    }
                    ...
                    public void onNext(List<UberRide> uberRides) {
                        Ln.d(
                                "Initial location observable emitted this data items: %s",
                                uberRides
                        );

                        if (uberRidesView != null) {
                            if (!uberRides.isEmpty()) {
                                uberRidesView.showRides(uberRides);
                                final UberRide uberRide = uberRides.get(0);
                                uberRidesView.drawRoute(uberRide.getPickupLat(), uberRide.getPickupLon(), uberRide.getDropOffLat(), uberRide.getDropOffLon());

                            } else {
                                uberRidesView.showNoRides();
                            }
                        }
                    }
                }
    ...
    @Override
    public void setView(UberRidesView view) {
        this.uberRidesView = view;
    }
