...
public class FusedLocationManager {
    ...
    private Location getLastKnownLocation() throws Exception {
        if (GooglePlayServicesUtil.isGooglePlayServicesAvailable(context) == ConnectionResult.SUCCESS) {
            ...
            try {
                ...
                ConnectionResult connectionResult = googleApiClient.blockingConnect(2, TimeUnit.MINUTES);
    ...
    public Observable<Location> getLocationWithFrequency(Frequency frequency) {
        return Observable.timer(0L,
                                frequency.getTimeBetweenPolling(),
                                TimeUnit.MILLISECONDS).flatMap(new Func1<Long, Observable<? extends Location>>() {
            @Override
    ...
    public final Observable<String> getCityWithLatitudeAndLongitude(final float latitude, final float longitude) {
